...
static char nv3_arb(nv3_fifo_info *res_info, nv3_sim_state *state,  nv3_arb_info *ainfo)
{
	...
	if (state->enable_mp)
	{
		mns = 1000000 * (mmisses * state->mem_page_miss + mburst_size / (state->memory_width / 8) + refresh_cycle) /
			  state->mclk_khz;
	...
	if (ainfo->gr_en)
	{
		...
		ainfo->first_gacc = 0;
		ainfo->first_macc = 1;
		gns = 1000000 * (gmisses * state->mem_page_miss + ainfo->gburst_size / (state->memory_width / 8) + refresh_cycle) /
			  state->mclk_khz;
	...
	if (ainfo->vid_en)
	{
		...
		ainfo->first_gacc = 1;
		ainfo->first_macc = 1;
		vns = 1000000 * (vmisses * state->mem_page_miss + ainfo->vburst_size / (state->memory_width / 8) + refresh_cycle) /
			  state->mclk_khz;
