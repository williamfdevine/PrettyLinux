...
static void matroxfb_cfb4_copyarea(struct fb_info *info, const struct fb_copyarea *area)
{
	...
	{
		matrox_accel_bmove_lin(minfo, minfo->fbcon.var.xres_virtual >> 1, area->sy, area->sx >> 1, area->dy, area->dx >> 1,
							   area->height, area->width >> 1);
	}
}
...
static void matroxfb_copyarea(struct fb_info *info, const struct fb_copyarea *area)
{
	...
	matrox_accel_bmove(minfo, minfo->fbcon.var.xres_virtual, area->sy, area->sx, area->dy, area->dx, area->height,
					   area->width);
}
...
static void matroxfb_fillrect(struct fb_info *info, const struct fb_fillrect *rect)
{
	...
	{
		case ROP_COPY:
			matroxfb_accel_clear(minfo, ((u_int32_t *)info->pseudo_palette)[rect->color], rect->dy, rect->dx, rect->height,
								 rect->width);
			break;
	}
}
...
static void matroxfb_cfb4_fillrect(struct fb_info *info, const struct fb_fillrect *rect)
{
	...
	{
		case ROP_COPY:
			matroxfb_cfb4_clear(minfo, ((u_int32_t *)info->pseudo_palette)[rect->color], rect->dy, rect->dx, rect->height,
								rect->width);
			break;
	}
}
