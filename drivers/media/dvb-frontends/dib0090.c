...
static void dib0090_reset_digital(struct dvb_frontend *fe, const struct dib0090_config *cfg)
{
	...
	if ((PllCfg & 0x1FFF) != ((cfg->io.pll_range << 12) | (cfg->io.pll_loopdiv << 6) | (cfg->io.pll_prediv)) &&
		(!cfg->in_soc)
		&& !cfg->io.pll_bypass)
	{
...
static int dib0090_fw_reset_digital(struct dvb_frontend *fe, const struct dib0090_config *cfg)
{
	...
	dib0090_fw_write_reg(state, 0x20,
						 ((cfg->io.adc_clock_ratio - 1) << 11) | (0 << 10) | (1 << 9) | (1 << 8) | (cfg->data_tx_drv << 4) |
						 cfg->ls_cfg_pad_drv);
...
static void dib0090_gain_apply(struct dib0090_state *state, s16 gain_delta, s16 top_delta, u8 force)
{
	...
	{
		if (g[0] == 0 ||
			ref < (g[1] - g[0]))	/* if total gain of the current amp is null or this amp is not concerned because it starts to work from an higher gain value */
		{
...
#ifdef DEBUG_AGC
	dprintk("GA CALC: DB: %3d(rf) + %3d(bb) = %3d gain_reg[0]=%04x gain_reg[1]=%04x gain_reg[2]=%04x gain_reg[0]=%04x", rf,
			bb, rf + bb,
		...
		else
		{
			...
			if (*tune_state == CT_AGC_STEP_1)  	/* quickly go to the correct range of the ADC power */
			{
				if (ABS(adc_error) < 50 || state->agc_step++ > 5)
				{
					...
					if (state->fe->dtv_property_cache.delivery_system == STANDARD_DAB)
					{
						dib0090_write_reg(state, 0x02, (1 << 15) | (15 << 11) | (31 << 6) |
										  (63));	/* cap value = 63 : narrow BB filter : Fc = 1.8MHz */
						dib0090_write_reg(state, 0x04, 0x0);
					}
					else
#endif
...
static int dib0090_reset(struct dvb_frontend *fe)
{
	...
	state->calibrate = DC_CAL | WBD_CAL |
					   TEMP_CAL;	/* enable iq-offset-calibration and wbd-calibration when tuning next time */
...
static void dib0090_set_bandwidth(struct dib0090_state *state)
{
	...
	if (state->identity.in_soc)
	{
		dib0090_write_reg(state, 0x05,
						  0x9bcf); /* attenuator_ibias_tri = 2 ; input_stage_ibias_tr = 1 ; nc = 11 ; ext_gm_trim = 1 ; obuf_ibias_trim = 4 ; filter13_gm2_ibias_t = 15 */
	}
	else
	{
		dib0090_write_reg(state, 0x02, (5 << 11) | (8 << 6) | (22 & 0x3f));	/* 22 = cap_value */
		dib0090_write_reg(state, 0x05,
						  0xabcd);	/* = 0xabcd : attenuator_ibias_tri = 2 ; input_stage_ibias_tr = 2 ; nc = 11 ; ext_gm_trim = 1 ; obuf_ibias_trim = 4 ; filter13_gm2_ibias_t = 13 */
	}
}
...
static int dib0090_captrim_search(struct dib0090_state *state, enum frontend_tune_state *tune_state)
{
	...
	else if (*tune_state == CT_TUNER_STEP_1)
	{
		...
		else
		{
			...
			if (state->rest == 0 ||
				state->identity.in_soc)  	/* Just for 8090P SOCS where auto captrim HW bug : TO CHECK IN ACI for SOCS !!! if 400 for 8090p SOC => tune issue !!! */
			{
				adc_target = 200;
			}
			else
...
static int dib0090_tune(struct dvb_frontend *fe)
{
	...
	if (*tune_state == CT_TUNER_START)
	{
		...
		state->calibrate = CAPTRIM_CAL;	/* captrim serach now */
			 ...
			 CT_TUNER_STEP_0)  	/* Warning : because of captrim cal, if you change this step, change it also in _cal.c file because it is the step following captrim cal state machine */
	{
		const struct dib0090_wbd_slope *wbd = state->current_wbd_table;
		...
		dprintk("VCOF in kHz: %d ((%d*%d) << 1))", (u32) ((pll->hfdiv * state->rf_request) * 2), (u32) pll->hfdiv,
...
struct dvb_frontend *dib0090_register(struct dvb_frontend *fe, struct i2c_adapter *i2c,
									  const struct dib0090_config *config)
{
...
struct dvb_frontend *dib0090_fw_register(struct dvb_frontend *fe, struct i2c_adapter *i2c,
		const struct dib0090_config *config)
{
