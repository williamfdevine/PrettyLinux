...
static int dib8096_set_param_override(struct dvb_frontend *fe)
{
	...
	{
		default:
			deb_info("Warning : Rf frequency  (%iHz) is not in the supported range, using VHF switch ",
			...
			for (loopdiv = 1; loopdiv < 64; loopdiv++)
			{
				...
				if ((fdem >= fdem_min) && (fmem <= fmem_max) && (fs >= fe->dtv_property_cache.bandwidth_hz / 1000))
				{
					...
					for (harmonic_id = (fe->dtv_property_cache.frequency / (1000 * fs));
						 harmonic_id <= ((fe->dtv_property_cache.frequency / (1000 * fs)) + 1); harmonic_id++)
					{
					...
					if (!spur)
					{
						...
						deb_info("RF %6d; BW %6d; Xtal %6d; Fmem %6d; Fdem %6d; Fs %6d; Prediv %2d; Loopdiv %2d; Timf %8d;",
								 fe->dtv_property_cache.frequency, fe->dtv_property_cache.bandwidth_hz, xtal, fmem, fdem, fs, prediv, loopdiv,
								 adc->timf);
						break;
					}
				}
