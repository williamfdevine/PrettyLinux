...
Amd7930_fill_Dfifo(struct IsdnCardState *cs)
{
	...
	if (cs->debug & L1_DEB_ISAC)
	{
		debugl1(cs, "Amd7930: fill_Dfifo, DMR3: 0x%02X, DTCR read: 0x%04X write: 0x%02X 0x%02X", dmr3, dtcrr, LOBYTE(dtcrw),
				HIBYTE(dtcrw));
	}
...
void Amd7930_interrupt(struct IsdnCardState *cs, BYTE irflags)
{
	...
	while (irflags)
	{
		...
		if (cs->debug & L1_DEB_ISAC)
		{
			debugl1(cs, "Amd7930: interrupt: flags: 0x%02X, DSR1: 0x%02X, DSR2: 0x%02X, LSR: 0x%02X, DER=0x%04X", irflags, dsr1,
					dsr2, lsr, der);
		}
	...
	if (test_bit(FLG_DBUSY_TIMER, &cs->HW_Flags))
	{
		...
		if (cs->debug & L1_DEB_ISAC)
		{
			debugl1(cs,
					...
					dsr1, dsr2, der, cs->tx_skb->len, cs->dc.amd7930.tx_xmtlen, dtcr, cs->tx_cnt);
		}
