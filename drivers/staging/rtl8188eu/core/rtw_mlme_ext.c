...
static void issue_beacon(struct adapter *padapter, int timeout_ms)
{
	...
	*(fctrl) = 0;
	...
	pframe = rtw_set_ie(pframe, _SUPPORTEDRATES_IE_, min_t(unsigned int, rate_len, 8), cur_network->SupportedRates,
						&pattrib->pktlen);
	...
	if (rate_len > 8)
	{
		pframe = rtw_set_ie(pframe, _EXT_SUPPORTEDRATES_IE_, (rate_len - 8), (cur_network->SupportedRates + 8),
							&pattrib->pktlen);
	}
...
static void issue_probersp(struct adapter *padapter, unsigned char *da)
{
	...
	*(fctrl) = 0;
	...
	if ((pmlmeinfo->state & 0x03) == WIFI_FW_AP_STATE)
	{
		pwps_ie = rtw_get_wps_ie(cur_network->IEs + _FIXED_IE_LENGTH_, cur_network->IELength - _FIXED_IE_LENGTH_, NULL,
								 &wps_ielen);
						  ...
						  struct sta_info *pstat, int pkt_type)
{
	...
	*(fctrl) = 0;
	...
	else
	{
		...
		pframe = rtw_set_ie(pframe, _EXT_SUPPORTEDRATES_IE_, (pstat->bssratelen - 8), pstat->bssrateset + 8,
							&(pattrib->pktlen));
	}
...
static void issue_assocreq(struct adapter *padapter)
{
	...
	*(fctrl) = 0;
	...
	/* SSID */
	pframe = rtw_set_ie(pframe, _SSID_IE_,  pmlmeinfo->network.Ssid.SsidLength, pmlmeinfo->network.Ssid.Ssid,
						&(pattrib->pktlen));
	...
	/* RSN */
	p = rtw_get_ie((pmlmeinfo->network.IEs + sizeof(struct ndis_802_11_fixed_ie)), _RSN_IE_2_, &ie_len,
				   (pmlmeinfo->network.IELength - sizeof(struct ndis_802_11_fixed_ie)));
	...
	{
		p = rtw_get_ie((pmlmeinfo->network.IEs + sizeof(struct ndis_802_11_fixed_ie)), _HT_CAPABILITY_IE_, &ie_len,
					   (pmlmeinfo->network.IELength - sizeof(struct ndis_802_11_fixed_ie)));
	...
	*(fctrl) = 0;
	...
	if (pmlmepriv->num_sta_no_ht > 0)
	{
		...
		while (phead != plist)
		{
			...
			p = rtw_get_ie(pbss_network->IEs + _FIXED_IE_LENGTH_, _HT_CAPABILITY_IE_, &len,
							 ...
							 struct recv_frame *precv_frame)
{
	...
	if (!memcmp(GetAddr3Ptr(pframe), pnetwork->MacAddress, ETH_ALEN))
	{
		if (pmlmeinfo->state & WIFI_FW_AUTH_NULL)
		{
			...
			pmlmeinfo->assoc_AP_vendor = check_assoc_AP(pframe + sizeof(struct ieee80211_hdr_3addr),
		...
		else if ((pmlmeinfo->state & 0x03) == WIFI_FW_ADHOC_STATE)
		{
			...
			else
			{
				...
				/* get supported rate */
				if (update_sta_support_rate(padapter, (pframe + WLAN_HDR_A3_LEN + _BEACON_IE_OFFSET_),
							   ...
							   struct recv_frame *precv_frame)
{
	...
	p = rtw_get_ie(pframe + WLAN_HDR_A3_LEN + ie_offset, _SUPPORTEDRATES_IE_, &ie_len,
				   pkt_len - WLAN_HDR_A3_LEN - ie_offset);
	...
	if (wpa_ie == NULL)
	{
		...
		if ((psecuritypriv->wpa_psk > 0) && (pstat->flags & (WLAN_STA_WPS | WLAN_STA_MAYBE_WPS)))
		{
			if (pmlmepriv->wps_beacon_ie)
			{
				...
				rtw_get_wps_attr_content(pmlmepriv->wps_beacon_ie, pmlmepriv->wps_beacon_ie_len, WPS_ATTR_SELECTED_REGISTRAR ,
