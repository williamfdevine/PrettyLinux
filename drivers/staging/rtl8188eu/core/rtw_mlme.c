...
static int rtw_is_desired_network(struct adapter *adapter, struct wlan_network *pnetwork)
{
	...
	{
		if (rtw_get_wps_ie(pnetwork->network.IEs + _FIXED_IE_LENGTH_, pnetwork->network.IELength - _FIXED_IE_LENGTH_, NULL,
						   &wps_ielen) != NULL)
		{
			return true;
		}
...
int rtw_restruct_wmm_ie(struct adapter *adapter, u8 *in_ie, u8 *out_ie, uint in_len, uint initial_out_len)
{
	...
	for (i = 12; i < in_len; i += (in_ie[i + 1] + 2) /* to the next IE element */)
	{
		...
		if (in_ie[i] == 0xDD && in_ie[i + 2] == 0x00 && in_ie[i + 3] == 0x50  && in_ie[i + 4] == 0xF2 && in_ie[i + 5] == 0x02 &&
			i + 5 < in_len)
		{
