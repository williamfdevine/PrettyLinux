...
int get_current_ap(struct ks_wlan_private *priv, struct link_ap_info_t *ap_info)
{
   ...
   rssi=%d\n    sq=%d\n    capability=%04X\n", ap->bssid[0], ap->bssid[1], ap->bssid[2], ap->bssid[3], ap->bssid[4],
			ap->bssid[5], &(ap->ssid.body[0]), ap->rate_set.body[0], ap->rate_set.body[1], ap->rate_set.body[2],
			ap->rate_set.body[3], ap->rate_set.body[4], ap->rate_set.body[5], ap->rate_set.body[6], ap->rate_set.body[7],
	...
	if (auth_type != TYPE_DATA && priv->wpa.rsn_enabled)
	{
		if (memcmp(&eth_hdr->h_source[0], &priv->eth_addr[0], ETH_ALEN))  	/* source address check */
		{
				...
				&& priv->wpa.key[auth_type - 1].key_len)
			{
				...
				{
					MichaelMICFunction(&michel_mic, (uint8_t *) priv->wpa.key[auth_type - 1].rx_mic_key, (uint8_t *) priv->rxp,
...
void hostif_phy_information_confirm(struct ks_wlan_private *priv)
{
   ...
   FCSErrorCount=%u\n", rssi, signal, LinkSpeed, TransmittedFrameCount, ReceivedFragmentCount, FailedCount,
			FCSErrorCount);
...
int hostif_data_request(struct ks_wlan_private *priv, struct sk_buff *packet)
{
	...
	if (priv->wpa.rsn_enabled && priv->wpa.key[0].key_len)
	{
		...
		else
		{
			...
			{
				MichaelMICFunction(&michel_mic, (uint8_t *) priv->wpa.key[0].tx_mic_key, (uint8_t *) & pp->data[0], (int)packet_len,
								   (uint8_t) 0,	/* priority */
								   (uint8_t *) michel_mic.
								   Result);
