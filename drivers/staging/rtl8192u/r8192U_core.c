	...
	if (priv->card_8192_version == (u8)VERSION_819xU_A)
	{
		...
		for (i = 0; i < 14; i++)
		{
			if (priv->EEPROM_Def_Ver == 0)
			{
				if (i <= 3)
				{
					priv->TxPowerLevelCCK[i] = priv->EEPROMTxPowerLevelOFDM24G[0] + (priv->EEPROMTxPowerLevelCCK -
				...
				{
					priv->TxPowerLevelCCK[i] = priv->EEPROMTxPowerLevelOFDM24G[2] + (priv->EEPROMTxPowerLevelCCK -
											   priv->EEPROMTxPowerLevelOFDM24G[1]);
				}
			}
			else if (priv->EEPROM_Def_Ver == 1)
