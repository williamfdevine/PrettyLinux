...
static struct cryptocop_dma_desc *alloc_cdesc(int alloc_flag)
{
	...
	cdesc->dma_descr = (dma_descr_data *)(((unsigned long int)cdesc + offsetof(struct cryptocop_dma_desc,
										   dma_descr_buf) + DESCR_ALLOC_PAD) & ~0x0000001F);
...
static int create_input_descriptors(struct cryptocop_operation *operation, struct cryptocop_tfrm_ctx *tc,
									struct cryptocop_dma_desc **id, int alloc_flag)
{
	int                        err = 0;
	...
	if (((tc->produced + tc->tcfg->inject_ix) > operation->tfrm_op.outlen) || (tc->produced &&
			(operation->tfrm_op.outlen == 0)))
	{
	...
	while ((outiov_ix < operation->tfrm_op.outcount) &&
		   ((out_ix + operation->tfrm_op.outdata[outiov_ix].iov_len) <= tc->tcfg->inject_ix))
	{
...
static int create_output_descriptors(struct cryptocop_operation *operation, int *iniov_ix, int *iniov_offset,
									 size_t desc_len, struct cryptocop_dma_desc **current_out_cdesc, struct strcop_meta_out *meta_out, int alloc_flag)
{
	while (desc_len != 0)
	{
		struct cryptocop_dma_desc  *cdesc;
	...
	(*current_out_cdesc)->dma_descr->wait =
...
static int append_input_descriptors(struct cryptocop_operation *operation, struct cryptocop_dma_desc **current_in_cdesc,
									struct cryptocop_dma_desc **current_out_cdesc, struct cryptocop_tfrm_ctx *tc, int alloc_flag)
{
...
static int cryptocop_setup_dma_list(struct cryptocop_operation *operation, struct cryptocop_int_operation **int_op,
									int alloc_flag)
{
	...
	(*int_op) = (struct cryptocop_int_operation *)(((unsigned long int)(iop_alloc_ptr + DESCR_ALLOC_PAD + offsetof(
					struct cryptocop_int_operation, ctx_out)) & ~0x0000001F) - offsetof(struct cryptocop_int_operation, ctx_out));
	...
	while (tcfg)
	{
		...
		{
			DEBUG_API(printk("cryptocop_setup_dma_list: transform id %d inject_ix (%d) > operation->tfrm_op.outlen(%d)", tcfg->tid,
	...
	if (cipher_ctx.tcfg && (cipher_ctx.tctx->init.alg != cryptocop_alg_mem2mem))
	{
		...
		if ((cipher_ctx.tctx->init.cipher_mode == cryptocop_cipher_mode_cbc) &&
			(cipher_ctx.tcfg->flags & CRYPTOCOP_EXPLICIT_IV))
		{
	...
	while (odsc)
	{
		...
		while (dcfg)
		{
			...
			if (tc->active && (tc->current_src != tc->previous_src))
			{
				...
				{
					DEBUG_API(
						printk("cryptocop_setup_dma_list: can only disconnect from or connect to a unit on a multiple of the blocklength, old: cons=%d, prod=%d, block=%d, new: cons=%d prod=%d, block=%d.\n",
							   tc->prev_src ? tc->prev_src->consumed : INT_MIN, tc->prev_src ? tc->prev_src->produced : INT_MIN,
							   tc->prev_src ? tc->prev_src->blocklength : INT_MIN, tc->curr_src ? tc->curr_src->consumed : INT_MIN,
							   tc->curr_src ? tc->curr_src->produced : INT_MIN, tc->curr_src ? tc->curr_src->blocklength : INT_MIN));
					failed = -EINVAL;
					goto error_cleanup;
				}
			}
		...
		*/
		...
		if (cipher_ctx.active)
		{
			...
			DEBUG(printk("set ciphsel=%d ciphconf=%d cbcmode=%d decrypt=%d\n", meta_out.ciphsel, meta_out.ciphconf,
						 meta_out.cbcmode, meta_out.decrypt));
		...
		failed = create_output_descriptors(operation, &iniov_ix, &iniov_offset, desc_len, &current_out_cdesc, &meta_out,
										   alloc_flag);
...
int cryptocop_free_session(cryptocop_session_id sid)
{
	...
	list_for_each_safe(node, tmp, &remove_list)
	{
		list_del(node);
		pj = list_entry(node, struct cryptocop_prio_job, node);
		pj->oper->operation_status =
			-EAGAIN;  /* EAGAIN is not ideal for job/session terminated but it's the best choice I know of. */
...
static int cryptocop_job_setup(struct cryptocop_prio_job **pj, struct cryptocop_operation *operation)
{
	...
	*pj = kmalloc(sizeof (struct cryptocop_prio_job), alloc_flag);
	...
	if (operation->use_dmalists)
	{
		...
		if (!operation->list_op.inlist || !operation->list_op.outlist || !operation->list_op.out_data_buf ||
		...
		(*pj)->iop = (struct cryptocop_int_operation *)(((unsigned long int)(iop_alloc_ptr + DESCR_ALLOC_PAD + offsetof(
						 struct cryptocop_int_operation, ctx_out)) & ~0x0000001F) - offsetof(struct cryptocop_int_operation, ctx_out));
...
static int map_pages_to_iovec(struct iovec *iov, int iovlen, int *iovix, struct page **pages, int nopages, int *pageix,
							  int *pageoffset, int map_length )
{
	int tmplen;
...
static int cryptocop_ioctl_process(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (oper.do_cipher)
	{
		...
		if ((oper.cipher_start < 0) || (oper.cipher_len <= 0) || (oper.cipher_start > oper.inlen) ||
			((oper.cipher_start + oper.cipher_len) > oper.inlen))
		{
	...
	if (oper.do_digest)
	{
		...
		if ((oper.digest_start < 0) || (oper.digest_len <= 0) || (oper.digest_start > oper.inlen) ||
			((oper.digest_start + oper.digest_len) > oper.inlen))
		{
	...
	if (oper.do_csum)
	{
		...
		if ((oper.csum_start < 0) || (oper.csum_len <= 0) || (oper.csum_start > oper.inlen) ||
			((oper.csum_start + oper.csum_len) > oper.inlen))
		{
	...
	noinpages = (((unsigned long int)(oper.indata + prev_ix) & ~PAGE_MASK) + oper.inlen - 1 - prev_ix + ~PAGE_MASK) >>
				PAGE_SHIFT;
	...
	if (oper.do_cipher)
	{
		nooutpages = (((unsigned long int)oper.cipher_outdata & ~PAGE_MASK) + oper.cipher_outlen - 1 + ~PAGE_MASK) >>
					 PAGE_SHIFT;
	...
	if (noinpages > 1)
	{
		...
		cop->tfrm_op.indata[0].iov_base = (unsigned char *)page_address(inpages[0]) + ((unsigned long int)(
											  oper.indata + prev_ix) & ~PAGE_MASK);
	...
	if (oper.do_cipher)
	{
		if (!map_pages_to_iovec(cop->tfrm_op.outdata, iovlen, &iovix, outpages, nooutpages, &pageix, &pageoffset,
								oper.cipher_outlen))
		{
...
static void print_user_dma_lists(struct cryptocop_dma_list_operation *dma_op)
{
	...
	printk("out_data_buf = 0x%p, phys_to_virt(out_data_buf) = 0x%p\n", dma_op->out_data_buf,
		   phys_to_virt((unsigned long int)dma_op->out_data_buf));
	printk("in_data_buf = 0x%p, phys_to_virt(in_data_buf) = 0x%p\n", dma_op->in_data_buf,
		   phys_to_virt((unsigned long int)dma_op->in_data_buf));
