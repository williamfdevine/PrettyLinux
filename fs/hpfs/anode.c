...
go_down:
	...
	if (n >= 0)
	{
		...
		if (hpfs_alloc_if_possible(s, se = le32_to_cpu(btree->u.external[n].disk_secno) + le32_to_cpu(
	...
	if (!(se = hpfs_alloc_sector(s, se, 1,
								 fsecno * ALLOC_M > ALLOC_FWD_MAX ? ALLOC_FWD_MAX : fsecno * ALLOC_M < ALLOC_FWD_MIN ? ALLOC_FWD_MIN : fsecno *
								 ALLOC_M)))
	{
		brelse(bh);
		return -1;
