...
static int snd_ca0106_pcm_prepare_playback(struct snd_pcm_substream *substream)
{
	...
	snd_ca0106_ptr_write(emu, PLAYBACK_PERIOD_SIZE, channel, frames_to_bytes(runtime,
						 runtime->period_size) << 16); // buffer size in bytes
...
static int snd_ca0106_pcm_prepare_capture(struct snd_pcm_substream *substream)
{
	...
	*/
	...
	snd_ca0106_ptr_write(emu, CAPTURE_BUFFER_SIZE, channel, frames_to_bytes(runtime,
		...
		int cmd)
{
	...
	switch (cmd)
	{
		...
		case SNDRV_PCM_TRIGGER_STOP:
			snd_ca0106_ptr_write(emu, BASIC_INTERRUPT, 0, snd_ca0106_ptr_read(emu, BASIC_INTERRUPT, 0) & ~(0x100 << channel));
			snd_ca0106_ptr_write(emu, EXTENDED_INT_MASK, 0, snd_ca0106_ptr_read(emu, EXTENDED_INT_MASK,
								 0) & ~(0x110000 << channel));
			epcm->running = 0;
			break;
