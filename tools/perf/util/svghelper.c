...
void svg_partial_wakeline(u64 start, int row1, char *desc1, int row2, char *desc2, const char *backtrace)
{
	...
	if (row1 < row2)
	{
		if (row1)
		{
			fprintf(svgfile,
					...
					time2pixels(start), row1 * SLOT_MULT + SLOT_HEIGHT,  time2pixels(start),
					row1 * SLOT_MULT + SLOT_HEIGHT + SLOT_MULT / 32);
	...
	else
	{
		if (row2)
		{
			fprintf(svgfile,
					"<line x1=\"%.8f\" y1=\"%.2f\" x2=\"%.8f\" y2=\"%.2f\" style=\"stroke:rgb(32,255,32);stroke-width:0.009\"/>\n",
					time2pixels(start), row2 * SLOT_MULT + SLOT_HEIGHT,  time2pixels(start),
					row2 * SLOT_MULT + SLOT_HEIGHT + SLOT_MULT / 32);
...
void svg_time_grid(double min_thickness)
{
	...
	while (i < last_time)
	{
		...
		if (thickness >= min_thickness)
			fprintf(svgfile, "<line x1=\"%.8f\" y1=\"%.2f\" x2=\"%.8f\" y2=\"%" PRIu64
